    ...
    conn.execute(
        "INSERT INTO entities
             (id, corpus_id, canonical_name, kind, abstract_kind, aliases, description,
              ...
              derived_at_kind, derived_at_sha, start_line, end_line)
         VALUES (
             ?1, ?2, ?3, ?4,
             CASE WHEN ?5 != '' THEN ?5
                  ELSE COALESCE((
             ...
             END,
             ?6, ?7, ?8, ?9, ?10, ?11, COALESCE(?12, 'concrete'), COALESCE(?13, ''), ?14, ?15)
         ON CONFLICT(id) DO UPDATE SET
             ...
             start_line        = COALESCE(entities.start_line, excluded.start_line),
             end_line          = COALESCE(entities.end_line, excluded.end_line)",
        params![
            entity.id,
            entity.corpus_id,
...
pub(crate) fn row_to_entity(row: &rusqlite::Row<'_>) -> rusqlite::Result<Entity> {
    ...
    //               appearance_count(9), confidence(10), derived_at_kind(11),
    //               derived_at_sha(12), start_line(13), end_line(14)
