...
pub fn upsert(db: &Database, p: &EntityPurpose) -> Result<()> {
    db.conn().execute(
        "INSERT OR REPLACE INTO entity_purposes
         (entity_id, corpus_id, purpose, model, generated_at)
         VALUES (?1, ?2, ?3, ?4, ?5)",
        params![p.entity_id, p.corpus_id, p.purpose, p.model, p.generated_at],
    )?;
    Ok(())
}

pub fn get(db: &Database, corpus_id: &str, entity_id: &str) -> Result<Option<EntityPurpose>> {
    let mut stmt = db.conn().prepare(
        "SELECT entity_id, corpus_id, purpose, model, generated_at
         FROM entity_purposes WHERE corpus_id = ?1 AND entity_id = ?2",
    )?;
