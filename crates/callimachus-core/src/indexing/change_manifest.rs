...
// ── Change-detection data types ─────────────────────────────────────────────
//
// `ChangeKind`, `ChangedSource`, and `CommitMeta` are part of the adapter
// contract (an adapter's `changed_sources` produces them), so they live in
// `callimachus-adapter-contract`. They are re-exported here at their historical
// `crate::indexing::change_manifest::…` path; `ChangeManifest` below — which is
// internal to the indexing engine — consumes them.
pub use callimachus_adapter_contract::{ChangeKind, ChangedSource, CommitMeta};
