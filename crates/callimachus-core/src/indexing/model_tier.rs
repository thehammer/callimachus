...
// ── Routing inputs ───────────────────────────────────────────────────────────
//
// `RoutingInputs` is part of the adapter contract (`static_routing_inputs`
// returns it), so it lives in `callimachus-adapter-contract`. It is re-exported
// here at its historical `crate::indexing::model_tier::…` path; the
// `ModelTierRouter` below — internal to the indexing engine — consumes it.
pub use callimachus_adapter_contract::RoutingInputs;
