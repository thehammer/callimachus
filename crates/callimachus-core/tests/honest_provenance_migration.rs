//! Schema-level tests covering migrations 013 and 018 (honest provenance).
//!
...
//!    new columns on `entities` exist after migrations 013+018.
...
//!    `derived_at_sha`.
//! 5. `legacy_derived_at_version_dropped` — migration 018 has dropped the
...
/// Migration 018 drops the `derived_at_version` and `superseded_at_version`
...
fn legacy_derived_at_version_dropped() {
    ...
    for table in &head_tables {
        assert!(
            ...
            "{table} must NOT have derived_at_version after migration 018"
        );
    }
    ...
    for table in &history_tables_with_superseded {
        assert!(
            ...
            "{table} must NOT have superseded_at_version after migration 018"
        );
    }
...
///
/// Migration 018 recreated the uniqueness index as a plain column-only index
...
fn history_uniqueness_indexes_are_plain_column_only() {
    ...
    for name in &index_names {
        ...
        assert!(
            ...
            "index {name} must not contain COALESCE after migration 018; got: {sql}"
        );
    }
}
