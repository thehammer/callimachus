...
| 8 | **themes** | Yes | Corpus-level architectural invariants. Asks the LLM to identify 3–7 cross-cutting patterns or invariants across a sample of entities. Skipped entirely when no dirty sources exist. Runs by default; can also be run standalone via `--pass theme`. Currently implemented for code and wiki adapters. Embed remains opt-in. |
